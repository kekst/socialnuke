...
class DiscordItem implements PlatformItem {
  ...
  constructor(
    ...
    private message: DiscordResultMessage
  ) {
    this.id = message.id;
  }
...
class DiscordTarget implements PlatformTarget {
      ...
      case "dms":
        {
          ...
          const recipients = dmChannel.recipients?.map(getUserName).join(", ") || "(empty)";
          const user = dmChannel.recipients?.[0];
          this.id = dmChannel.id;
          this.name = dmChannel.recipients?.length !== 1 ? `Group DM: ${recipients}` : recipients;
          this.type = "dms";
  ...
  async children?(): Promise<PlatformResponse<PlatformTarget[]>> {
    ...
    for (const channel of topLevel) {
      targets.push(new DiscordTarget(this.discordUser, this.token, "channel", channel, this.id));

      if (channel.type === 4) {
        const channels = json.filter((chan) => chan.parent_id === channel.id);
        for (const channel of channels) {
          targets.push(
            new DiscordTarget(this.discordUser, this.token, "channel", channel, this.id)
          );
        }
      }
    }
  ...
  private searchParams(filters: DiscordPlatformFilters): URLSearchParams {
    ...
    if (filters.has && filters.has !== "undefined") {
  ...
  private async search(params: URLSearchParams): Promise<PlatformResponse<DiscordResults>> {
    ...
    if (typeof json.document_indexed !== "undefined") {
      ...
      store.toast("info", `Not indexed yet, retrying in ${retryAfter} seconds...`);
      return {
        type: "action",
  ...
  async estimate(filters: DiscordPlatformFilters): Promise<PlatformResponse<number>> {
  ...
  async *query(
    ...
    while (true) {
      try {
        ...
        if (e.message === "No") {
          store.toast("error", `Unrecoverable error.`);
          throw new Error("Failure");
        }

        store.toast("info", `Recoverable error, retrying in 1 second...`);
        yield {
          type: "action",
          action: { type: "wait", time: 1000 }
        };
  ...
  async targets(type: string): Promise<PlatformResponse<PlatformTarget[]>> {
    if (type === "dms") {
      ...
      for (const channel of sorted) {
        ...
        targets.push(new DiscordTarget(this.discordUser, this.token, "dms", channel));
      }
    ...
    if (type === "guilds") {
      ...
      for (const guild of json) {
        targets.push(new DiscordTarget(this.discordUser, this.token, "guilds", guild));
      }
      return { type: "ok", result: targets };
    }
